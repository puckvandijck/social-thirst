...
def generate_ideas_openai(
    ...
    platform_choice = st.session_state.get("platform_choice", "Geen voorkeur")
    ...
    elif platform_choice and platform_choice != "Geen voorkeur":
        products_section = (
...
def main():
    ...
    st.write(
        "Kies een platform om te pushen in het dropdown-menu of vul zelf handmatig één of meerdere producten in. "
    ...
    def load_platform_catalog() -> pd.DataFrame:
        """Load the Andrelon platformen Excel located in the project folder."""
        try:
            df = pd.read_excel("Andrelon platformen.xlsx", engine="openpyxl")
            if "Range" not in df.columns:
                raise ValueError("Kolom 'Range' ontbreekt in Excel.")
            return df
        except Exception as e:
            st.warning(f"⚠️ Kon platformbestand niet laden: {e}")
            # Return empty placeholder so app still runs
            return pd.DataFrame(columns=["Range", "SKU", "Description"])

    catalog_df = load_platform_catalog()

    # --- Platform dropdown ---
    platform_options = ["Geen voorkeur"]
    if not catalog_df.empty:
        unique_platforms = sorted(df["Range"].dropna().unique().tolist())
    ...
    st.markdown("#### Handmatig product(en) toevoegen (optioneel)")
    ...
    if platform_choice != "Geen voorkeur" and not catalog_df.empty:
    ...
    # Store merged products in session
