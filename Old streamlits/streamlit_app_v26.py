...
def export_ideas_to_docx(df, brand_name, output_path="tiktok_ideas.docx"):
    ...
    # --- Title page ---
    title = doc.add_heading(f"TikTok Contentideeën – {brand_name}", level=0)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER

    doc.add_paragraph(
        "Gegenereerd met de Streamlit TikTok Content Tool.\n"
        "Elk idee bevat een trendfocus en een korte uitleg waarom deze trend actueel is."
    )
    ...
    for i, row in df.iterrows():
        ...
        doc.add_paragraph("")  # spacing line
