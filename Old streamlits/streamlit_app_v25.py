...
def generate_ideas_openai(
    ...
    Geef het resultaat als JSON-lijst van objecten met deze velden:
    title, hook, script, filming_tips, hashtags, sound, commercial_ok, short_explanation.
    """
    ...
    try:
        ...
        try:
            ...
            import ast
            ideas_list = ast.literal_eval(reply)

        records = []
        for idea in ideas_list[:n_ideas]:
            records.append({
                "Titel": idea.get("title", ""),
                "Hook": idea.get("hook", ""),
                "Script": idea.get("script", ""),
                "Filming tips": "; ".join(idea.get("filming_tips", [])) if isinstance(idea.get("filming_tips"), list) else str(idea.get("filming_tips", "")),
                "Hashtags": ", ".join(idea.get("hashtags", [])) if isinstance(idea.get("hashtags"), list) else str(idea.get("hashtags", "")),
                "Sound": idea.get("sound", ""),
                "Commercial OK": "Ja" if idea.get("commercial_ok", True) else "Nee",
                "Waarom actueel": idea.get("short_explanation", "")
            })

        return pd.DataFrame(records)

    except Exception as e:
        st.error(f"OpenAI call mislukte: {e}")
    ...
    if st.button("Genereer ideeën"):
        ...
        if ideas_df is not None and not ideas_df.empty:
            ...
            with col1:
                st.download_button(
                    "Download als CSV",
                    data=ideas_df.to_csv(index=False).encode("utf-8"),
                    file_name="tiktok_ideas.csv",
                    mime="text/csv",
                )
            with col2:
                docx_filename = "tiktok_ideas.docx"
                export_ideas_to_docx(ideas_df, docx_filename)
                st.download_button(
                    "Download als DOCX",
                    data=open(docx_filename, "rb").read(),
                    file_name=docx_filename,
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                )
        else:
            st.warning("Er zijn geen ideeën gegenereerd. Controleer je API-sleutel of probeer het opnieuw.")

def export_ideas_to_docx(df: pd.DataFrame, filename: str) -> None:
    from docx import Document
    from docx.shared import Pt

    doc = Document()
    doc.add_heading("TikTok Content Ideeën", level=1)

    for idx, row in df.iterrows():
        doc.add_heading(f"Idee {idx + 1}: {row['Titel']}", level=2)
        ...
        doc.add_paragraph(f"Filmtips: {row.get('Filming tips', '')}")
        doc.add_paragraph(f"Hashtags: {row.get('Hashtags', '')}")
        doc.add_paragraph(
            f"Sound: {row.get('Sound', '')} (Commercieel toegestaan: {row.get('Commercial OK', '')})"
        )
        doc.add_page_break()
